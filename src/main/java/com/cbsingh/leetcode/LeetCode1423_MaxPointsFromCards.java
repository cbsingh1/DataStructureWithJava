...
public class LeetCode1423_MaxPointsFromCards {
    public static int maxScore(int[] cardPoints, int k) {
        int maxScore=0, arrSum=0, windowSum=0;
        for (int cardPoint : cardPoints)
            arrSum += cardPoint;
        if(k==cardPoints.length || k > cardPoints.length)
            return arrSum;
        int i=0, j=0;
        while (j < cardPoints.length) {
            windowSum += cardPoints[j];
            if(j-i+1 == (cardPoints.length - k)) {
                if(maxScore < arrSum - windowSum)
                    maxScore = arrSum -windowSum;
    ...
    public static void main(String[] args) {
        System.out.println(maxScore(new int[] {10,2,3,4,5,6,1}, 3));
    }
}
